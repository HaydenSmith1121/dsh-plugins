...
（`catalog/overrides/reviewed.json` 的 `peerVerdict` / `peerNote`）为准。
...
> （已审核层），或集合仓库的 `plugins/<id>/plugin.json`（自研层）。
...
- **`@dsh-external/dsh-ads`**（已审核层）：`@deepseek-ai/dsh-client-locale ^0.1.0-rc.6`
  按预发布匹配规则不包含 `0.1.6-alpha.1`，同样是 `autoInstallPeers: false` 下的警告，
  实测启动无致命错误。完整证据见 `catalog/overrides/reviewed.json`。
