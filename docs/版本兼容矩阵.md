...
> 自研 6 个现在各自在自己的源码仓库里（原[插件集合仓库](https://github.com/HaydenSmith1121/dsh-plugin-collection) 已于 2026-09-18 退役），
> 第三方按上游规格安装。保留这张表是因为**这些数是实测出来的**，peer 提示的口径出自这里：
> 精确 pin 到**比本机更新**的版本值得提示（可能整棵插件树加载失败），pin 到更低版本不提示 ——
> 0.6.0 起这些只是安装前的 `notes`，不再分「致命 / 警告」，也不拦安装。
> 要看**当前**逐插件的 peer 结论，请查：
...
以上均为**真机实测**结论；第三方包的结论附在各自 `catalog/overrides/curated.json` 的
`notes` / `evidence` 里（环境 / 装法 / 四层校验输出 / 共存回归）。
...
> 预检与安装脚本读它来判定；「已装」页的体检读它报环境层的问题 —— 只报问题，不拦插件。
